...
function getRankLabel(stars: number) {
  if (stars === 3) return { label: "🏆 Tuyệt vời!", color: "text-orange-500" };
  if (stars === 2) return { label: "🌟 Giỏi lắm!", color: "text-sky-600" };
  if (stars === 1)
    return { label: "👍 Cố gắng thêm!", color: "text-purple-600" };
...
export function GameResultPage() {
  ...
  const total = state?.total ?? lesson?.games.length ?? 2;
  const stars = getStarCount(correct, total);
  const rank = getRankLabel(stars);
  const xpEarned =
    stars > 0
      ? (lesson?.games.reduce((sum, g) => sum + g.xpReward, 0) ?? 5)
      : 0;
  ...
  useEffect(() => {
    if (!saved && stars > 0 && lesson) {
      markGameComplete(lessonId, xpEarned);
      setSaved(true);
    }
      ...
      <div className="relative z-10 w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden animate-kids-bounce-in">
        ...
        <div className="relative z-10 px-6 py-6 flex flex-col items-center gap-5 text-center">
          ...
          <div className="w-full flex flex-col gap-3">
            <button
              onClick={() => navigate("/student")}
              className="w-full py-4 bg-gradient-to-r from-orange-400 to-pink-500 text-white text-lg font-extrabold rounded-2xl shadow-[0_5px_0_rgba(0,0,0,0.2)] active:translate-y-[3px] active:shadow-none transition-all flex items-center justify-center gap-2"
            >
              <Map size={20} /> Quay lại phòng học
            </button>

            <button
              onClick={() => navigate(`/student/game/${lessonId}`)}
              className="w-full py-3 border-2 border-gray-200 text-gray-600 font-extrabold rounded-2xl hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
            >
