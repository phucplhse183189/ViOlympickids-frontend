...
const WORLD_THEMES = [
  {
    ...
    cardColors: [
      ...
      { bg: "linear-gradient(135deg, #fb923c, #f97316)", border: "#c2410c" },
...
export function Math2B46QuizPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Production API tra cứu theo gameType "math2-quiz-3d" (slug math2-b46 cần bản API mới)
      for (const slug of ["math2-quiz-3d", "math2-b46"]) {
        try {
          const qs = await lessonService.getQuiz(slug);
          ...
          console.error(`getQuiz(${slug}) failed:`, err);
        }
      }
  ...
  const handleNext = () => {
    ...
    setTimeout(() => {
      ...
      if (currentIdx + 1 >= questions.length) {
        ...
        if (childId) {
          leaderboardService.submitAttempt(childId, questions[0]?.lessonId || "math2-b46", correctCount, questions.length).catch(console.error);
        }
        navigate("/student/result/math2-b46", { state: { correct: correctCount, total: questions.length } });
        return;
      }
      setCurrentIdx((i) => i + 1);
  ...
  if (loadState === "error" || questions.length === 0 || !q) {
    return (
      <div className="flex flex-col justify-center items-center gap-4 h-screen bg-slate-900 text-white px-6 text-center">
        <p className="text-xl font-bold">Chưa tải được câu hỏi quiz Bài 46.</p>
        <p className="text-sm text-slate-400 max-w-md">
          Có thể dữ liệu quiz chưa được cập nhật trên server. Hãy thử lại sau hoặc liên hệ giáo viên.
        </p>
        <button
          type="button"
          onClick={() => navigate("/student/game/math2-quiz-3d-shapes")}
          className="px-6 py-3 rounded-2xl bg-sky-500 font-bold hover:bg-sky-400 transition-colors"
        >
          Quay lại Bài 46
        </button>
      </div>
        ...
        <div className="hide-scrollbar" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "0 20px 40px", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px", marginTop: "4px" }}>
            <span style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.3), rgba(255,255,255,0.1))", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.5)", borderRadius: "999px", padding: "8px 24px", fontSize: "0.85rem", fontWeight: 900, letterSpacing: "0.05em", textTransform: "uppercase" as const, boxShadow: "0 6px 20px rgba(0,0,0,0.15), inset 0 1px 0 rgba(255,255,255,0.6)", textShadow: "0 2px 4px rgba(0,0,0,0.3)" }}>
              {theme.icon} {theme.name} · Bài 46: Khối trụ và Khối cầu
            </span>
          </div>
