...
export function AdminFeedbackPage() {
  ...
  return <div className="space-y-5 pb-8"><AdminPageHero eyebrow="Feedback operations" title="Lắng nghe để cải thiện" description="Tìm kiếm, phân loại và xử lý phản hồi của phụ huynh trong một quy trình thống nhất." icon={MessageSquare} metrics={[{ label: "Tổng góp ý", value: feedbacks.length, icon: MessagesSquare }, { label: "Chờ duyệt", value: feedbacks.filter((item) => item.status === "pending").length, icon: Clock }]} />
    <section className="rounded-[22px] border border-border bg-card p-3 shadow-sm"><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-12"><AdminSearch className="md:col-span-2 xl:col-span-4" value={search} onChange={setSearch} placeholder="Tên, số điện thoại hoặc nội dung…" /><AdminFilterSelect className="xl:col-span-2" label="Trạng thái" value={status} onChange={setStatus} options={[{ value: "all", label: "Tất cả trạng thái" }, ...Object.entries(statusMeta).map(([value, meta]) => ({ value: value as service.FeedbackStatus, label: meta.label }))]} /><AdminFilterSelect className="xl:col-span-2" label="Thể loại" value={category} onChange={setCategory} options={[{ value: "all", label: "Tất cả thể loại" }, ...Object.entries(categoryMeta).map(([value, label]) => ({ value: value as service.FeedbackCategory, label }))]} /><AdminFilterSelect className="xl:col-span-2" label="Số sao" value={rating} onChange={setRating} options={[{ value: "all", label: "Tất cả sao" }, { value: "5", label: "5 sao" }, { value: "4", label: "4 sao" }, { value: "3", label: "3 sao" }, { value: "2", label: "2 sao" }, { value: "1", label: "1 sao" }]} /><AdminDatePicker className="xl:col-span-2" label="Gửi từ ngày" value={date} onChange={setDate} /></div></section>
